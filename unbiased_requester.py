...
st.session_state.lang_bias_query = language_bias_tool(st.session_state.original_query)
st.session_state.pub_bias_query = publication_bias_tool(st.session_state.original_query)
...
if st.checkbox("Apply language-bias-tool"):
    ...
    if st.checkbox("\t Show language-bias-tool additional records"):
        ...
        data_lang_diff_to_display = data_lang_diff.drop(columns=columns_to_hide)
...
if st.checkbox("Apply publication-bias-tool"):
    ...
    if st.checkbox("\t Show publication-bias-tool data additional records"):
        ...
        data_pub_diff_to_display = data_pub_diff.drop(columns=columns_to_hide)
...
if st.checkbox("Apply localization-bias-tool"):
    ...
    data_localized_to_display = data_localized.drop(columns=columns_to_hide)
...
if st.checkbox("Apply availability-bias-tool"):
    ...
    data_available_summary = (
        data_available.groupby("openaccess")
        .agg({"dc:identifier": "count"})
        .reset_index()
    )
    nr_open_access_records = data_available_summary[
        data_available_summary["openaccess"]
    ].iloc[0, 1]
